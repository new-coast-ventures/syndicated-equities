...
class Property < ActiveRecord::Base
  ...
  def export_investors
    headers = %w{FirstName LastName Email AltEmail City State Zip InvestorEntity ClosedInvestment}
    csv = CSV.generate(headers: true) do |csv|
      csv << headers
      investments.each do |inv|
        user = inv.investor
        csv << [user&.first_name, user&.last_name, user&.email, user&.email_2, user&.city, user&.state, user&.zip_code, user&.csv_investor_entity, inv.closed]
      end
    end
